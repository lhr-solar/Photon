			...
			gui->begin_content("content", GetContentRegionAvail() - SCALE(15, 15), { 15, 15 }, { 15, 15 });
			{
				if (var->c_selection.selection_active == 0)
				{
					gui->begin_group();
					{
						...
						gui->begin_child("rgb");
						{
							...
							if (widget->checkbox_with_key("LHRS", &var->c_ragebot.ragebot, &var->c_ragebot.rage_key, &var->c_ragebot.rage_holding, &var->c_ragebot.rage_value, &var->c_ragebot.rage_show_binds))
							{
							...
							widget->checkbox("sa", &var->c_ragebot.silent_aimbot);

							widget->separator();

							widget->checkbox("hc", &var->c_ragebot.hit_chance);

							widget->separator();

							widget->slider_int("fv", &var->c_ragebot.fov, -180, 180, 1, "%d\xC2\xB0");
						}
						gui->end_child();
						...
						gui->begin_child("other");
						{
							...
							widget->dropdown("ba", &var->c_other.body_selection, var->c_other.body_list, var->c_other.body_list.size());

							widget->separator();

							widget->dropdown("sp", &var->c_other.points_selection, var->c_other.points_list, var->c_other.points_list.size());
						...
						gui->begin_child("rcl");
						{
							...
							widget->checkbox_with_key("er", &var->c_recoil.enable_recoil, &var->c_recoil.recoil_key, &var->c_recoil.recoil_holding, &var->c_recoil.recoil_value, &var->c_recoil.recoil_show_binds);

							widget->separator();

							widget->slider_int("smth", &var->c_recoil.smoothness, 0, 100, 1, "%d%%");
					...
					gui->begin_group();
					{
						...
						gui->begin_child("move");
						{
							...
							widget->slider_int("HC", &var->c_move.hit_chance, 0, 100, 1, "%d%%");
							{
								widget->set_tooltip("hc", "It displays a tooltip when you hover over an item, a very handy GUI utility\nwith many functions that are not always easy to understand by name.");
							}

							widget->separator();

							widget->slider_int("MM", &var->c_move.max_misses, 0, 100, 1, "%d%%");

							widget->separator();

							widget->checkbox_with_key("sps", &var->c_move.point_scale, &var->c_move.point_key, &var->c_move.point_holding, &var->c_move.point_value, &var->c_move.point_show_binds);

							widget->separator();

							widget->checkbox("hsil", &var->c_move.head_safety);
						...
						gui->begin_child("tr");
						{
							...
							widget->checkbox_with_key("etr", &var->c_trigger.enable_trigger, &var->c_trigger.trigger_key, &var->c_trigger.trigger_holding, &var->c_trigger.trigger_value, &var->c_trigger.trigger_show_binds);

							widget->separator();

							widget->checkbox("etrs", &var->c_trigger.trigger_in_smoke);

							widget->separator();

							widget->button("btt", { GetContentRegionAvail().x, SCALE(35) });
						...
						gui->begin_child("settings");
						{
							...
							widget->checkbox("HSIL", &var->c_settings.head_safety);
						}
						gui->end_child();
				...
				else if (var->c_selection.selection_active == 3)
				{
					gui->begin_group();
					{
						...
						gui->begin_child("esp");
						{
							widget->checkbox_with_key("EE", &var->c_esp.esp, &var->c_esp.esp_key, &var->c_esp.esp_holding, &var->c_esp.esp_value, &var->c_esp.esp_show_binds);

							widget->separator();

							widget->checkbox("TW", &var->c_esp.through_walls);

							widget->separator();

							widget->dropdown("DT", &var->c_esp.tracers_selection, var->c_esp.tracers_list, var->c_esp.tracers_list.size());

							widget->separator();

							widget->checkbox("DB", &var->c_esp.dynamic_boxes);

							widget->separator();

							widget->checkbox_with_color("IGR", &var->c_esp.ingame_radar, var->c_esp.inagame_color, true);
						}
						gui->end_child();

						gui->begin_child("GL");
						{
							widget->checkbox_with_key("EG", &var->c_glow.glow, &var->c_glow.glow_key, &var->c_glow.glow_holding, &var->c_glow.glow_value, &var->c_glow.glow_show_binds);

							widget->separator();

							widget->slider_int("TPB", &var->c_glow.power, 0, 100, 1, "%d%$");
						}
						gui->end_child();

						gui->begin_child("att");
						{
							widget->checkbox_with_color("AT", &var->c_attachments.attachments, var->c_attachments.attachments_color, true);

							widget->separator();

							widget->checkbox_with_key("VT", &var->c_attachments.teammates, &var->c_attachments.teammates_key, &var->c_attachments.teammates_holding, &var->c_attachments.teammates_value, &var->c_attachments.teammates_show_binds);
						}
						gui->end_child();
					...
					gui->begin_group();
					{
						...
						{
							widget->checkbox_with_key("EC", &var->c_chams.chams, &var->c_chams.chams_key, &var->c_chams.chams_holding, &var->c_chams.chams_value, &var->c_chams.chams_show_binds);

							widget->separator();

							widget->checkbox_with_color("BT", &var->c_chams.backtrack, var->c_chams.backtrack_color, true);

							widget->separator();

							widget->checkbox_with_color("OS", &var->c_chams.onshot, var->c_chams.onshot_color, true);

							widget->separator();

							widget->checkbox_with_color("RD", &var->c_chams.ragdolls, var->c_chams.ragdolls_color, true);
						...
						gui->begin_child("SK");
						{
							widget->checkbox("sk", &var->c_skeleton.skeleton);

							widget->separator();

							widget->dropdown("snpl", &var->c_skeleton.snaplines_selection, var->c_skeleton.snaplines_list, var->c_skeleton.snaplines_list.size());

							widget->separator();

							widget->checkbox_with_color("wp", &var->c_skeleton.weapon, var->c_skeleton.weapon_color, true);

							widget->separator();

							widget->checkbox_with_color("nn", &var->c_skeleton.nickname, var->c_skeleton.nickname_color, true);
						}
						gui->end_child();
