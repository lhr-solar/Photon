...
void GUI::init(GPU* gpu, Network* network, Parse* parse){
    ...
    sceneModel.addModel("track_glb", track_glb, track_glb_size, false);
...
void GUI::queueStartStreamForward(){
    NetworkCommand command{};
    command.type = NetworkCommandType::StartStreamForward;
    guiCommands.write([&](NetworkCommand& slot){ slot = command; });
}
...
void GUI::queueStopStreamForward(){
    NetworkCommand command{};
    command.type = NetworkCommandType::StopStreamForward;
    guiCommands.write([&](NetworkCommand& slot){ slot = command; });
}
...
void GUI::sceneWindow(){
    if (sceneModel.trackedObjectIndex >= 0
        ...
        SceneObject& trackedObject = sceneModel.objects[sceneModel.trackedObjectIndex];
        const bool useAssettoCoordinates = network != nullptr
            && parse != nullptr
            && network->activeConfig.kind == ProtocolKind::AssettoCorsa
            && parse->activeDBC == DBCKind::AssettoCorsa;
        if(useAssettoCoordinates){
            Position previousPosition = trackedObject.position;
            void* data = nullptr;
            uint32_t size = 0;
            parse->arena.read(0x037, 0, &data, &size);
            if(data && size >= sizeof(double)) trackedObject.position.x 
                = static_cast<float>(static_cast<double*>(data)[(size / sizeof(double)) - 1]);
            parse->arena.read(0x038, 0, &data, &size);
            if(data && size >= sizeof(double)) trackedObject.position.z 
                = static_cast<float>(static_cast<double*>(data)[(size / sizeof(double)) - 1]);
            parse->arena.read(0x039, 0, &data, &size);
            if(data && size >= sizeof(double)) trackedObject.position.y 
                = static_cast<float>(static_cast<double*>(data)[(size / sizeof(double)) - 1]);
            trackedObject.position.x *= 0.1f;
            trackedObject.position.z *= 0.1f;
            trackedObject.position.z += 0.3f;
            trackedObject.position.y *= -0.1f;

            const float dx = trackedObject.position.x - previousPosition.x;
            const float dy = trackedObject.position.y - previousPosition.y;
            const float movementSq = dx * dx + dy * dy;
            if(hasLastTrackedPosition && movementSq > 1.0e-6f){
                trackedObject.rotationDegrees = glm::degrees(std::atan2(dy, dx)) + 90.0f;
            }
    ...
    if (ImGui::Begin("scene", nullptr, ImGuiWindowFlags_NoTitleBar)) {
        ...
        if (ready) {
            ...
            const bool hasTrackedObject = sceneModel.trackedObjectIndex >= 0
                && sceneModel.trackedObjectIndex < static_cast<int>(sceneModel.objects.size());
            const Position trackedPosition = hasTrackedObject
                ? sceneModel.objects[sceneModel.trackedObjectIndex].position
                : Position{};
            ...
            const ImVec2 textMin(imageMin.x + 12.0f, imageMax.y - textSize.y - textPadding.y * 2.0f - 12.0f);
            ...
            drawList->AddRectFilled(
                textMin,
                textMax,
                ImGui::GetColorU32(ImVec4(0.10f, 0.10f, 0.11f, 0.88f)),
                6.0f);
            drawList->AddText(
                ImVec2(textMin.x + textPadding.x, textMin.y + textPadding.y),
...
void GUI::networkWindow(){
    if(ImGui::Begin("network", nullptr, ImGuiWindowFlags_NoTitleBar)){
        ...
        const bool protocolRunning = network != nullptr && network->networkStreamRunning.load(std::memory_order_acquire);
        const bool streamForwardRunning = network != nullptr && network->streamForwardRunning.load(std::memory_order_acquire);
        ...
        switch(protocolConfig.kind){
            ...
            case ProtocolKind::AssettoCorsa:
                ImGui::TextUnformatted("Assetto Corsa uses the built-in local UDP handshake on 127.0.0.1:9996.");
                break;
        ...
        ImGui::SameLine();
        ImGui::BeginDisabled(streamForwardRunning);
        if(ImGui::Button("stream my data")) queueStartStreamForward();
        ImGui::EndDisabled();
        ImGui::SameLine();
        ImGui::BeginDisabled(!streamForwardRunning);
        if(ImGui::Button("Stop Streaming")) queueStopStreamForward();
        ImGui::EndDisabled();
        ImGui::SameLine();
        if(ImGui::Button("Clear Messages")) handlerMessages.clear();
        ImGui::Spacing();
        if(!protocolRunning){
            ImGui::TextUnformatted("No source stream is active.");
        } else if(streamForwardRunning){
            ImGui::TextUnformatted("Forwarding current stream to 3.141.38.115:6200.");
        }
...
void GUI::setStyle(){
    ...
    nodeStyle.Colors[ImNodesCol_NodeBackground] = packNodeColor(panelBg);
    nodeStyle.Colors[ImNodesCol_NodeBackgroundHovered] = packNodeColor(elevatedBg);
    nodeStyle.Colors[ImNodesCol_NodeBackgroundSelected] = packNodeColor(ImVec4{elevatedBg.x, elevatedBg.y, elevatedBg.z, 1.00f});
    nodeStyle.Colors[ImNodesCol_NodeOutline] = packNodeColor(border);
    nodeStyle.Colors[ImNodesCol_TitleBar] = packNodeColor(ImVec4{0.11f, 0.10f, 0.09f, 1.00f});
    nodeStyle.Colors[ImNodesCol_TitleBarHovered] = packNodeColor(ImVec4{0.14f, 0.13f, 0.11f, 1.00f});
    nodeStyle.Colors[ImNodesCol_TitleBarSelected] = packNodeColor(accent);
    nodeStyle.Colors[ImNodesCol_Link] = packNodeColor(ImVec4{accent.x, accent.y, accent.z, 0.72f});
    nodeStyle.Colors[ImNodesCol_LinkHovered] = packNodeColor(accentHover);
    nodeStyle.Colors[ImNodesCol_LinkSelected] = packNodeColor(accent);
    nodeStyle.Colors[ImNodesCol_Pin] = packNodeColor(ImVec4{accent.x, accent.y, accent.z, 0.78f});
    nodeStyle.Colors[ImNodesCol_PinHovered] = packNodeColor(accentHover);
    nodeStyle.Colors[ImNodesCol_BoxSelector] = packNodeColor(ImVec4{accent.x, accent.y, accent.z, 0.16f});
    nodeStyle.Colors[ImNodesCol_BoxSelectorOutline] = packNodeColor(ImVec4{accent.x, accent.y, accent.z, 0.58f});
    nodeStyle.Colors[ImNodesCol_GridBackground] = packNodeColor(baseBg);
    nodeStyle.Colors[ImNodesCol_GridLine] = packNodeColor(ImVec4{border.x, border.y, border.z, 0.35f});
    nodeStyle.Colors[ImNodesCol_GridLinePrimary] = packNodeColor(ImVec4{dimText.x, dimText.y, dimText.z, 0.22f});
    nodeStyle.Colors[ImNodesCol_MiniMapBackground] = packNodeColor(ImVec4{baseBg.x, baseBg.y, baseBg.z, 0.88f});
    nodeStyle.Colors[ImNodesCol_MiniMapBackgroundHovered] = packNodeColor(ImVec4{panelBg.x, panelBg.y, panelBg.z, 0.94f});
    nodeStyle.Colors[ImNodesCol_MiniMapOutline] = packNodeColor(border);
    nodeStyle.Colors[ImNodesCol_MiniMapOutlineHovered] = packNodeColor(accent);
    nodeStyle.Colors[ImNodesCol_MiniMapNodeBackground] = packNodeColor(ImVec4{dimText.x, dimText.y, dimText.z, 0.35f});
    nodeStyle.Colors[ImNodesCol_MiniMapNodeBackgroundHovered] = packNodeColor(ImVec4{textColor.x, textColor.y, textColor.z, 0.65f});
    nodeStyle.Colors[ImNodesCol_MiniMapNodeBackgroundSelected] = packNodeColor(accent);
    nodeStyle.Colors[ImNodesCol_MiniMapNodeOutline] = packNodeColor(border);
    nodeStyle.Colors[ImNodesCol_MiniMapLink] = packNodeColor(ImVec4{accent.x, accent.y, accent.z, 0.65f});
    nodeStyle.Colors[ImNodesCol_MiniMapLinkSelected] = packNodeColor(accentHover);
    nodeStyle.Colors[ImNodesCol_MiniMapCanvas] = packNodeColor(ImVec4{textColor.x, textColor.y, textColor.z, 0.08f});
    nodeStyle.Colors[ImNodesCol_MiniMapCanvasOutline] = packNodeColor(ImVec4{textColor.x, textColor.y, textColor.z, 0.45f});
};
