...
bool synthesizeClassicBtr(uint32_t bitrateKbps, float samplePointPercent, uint32_t prescaler,
  ...
  const uint32_t btr1 =
      (static_cast<uint32_t>(tseg2 - 1) << 4) | static_cast<uint32_t>((tseg1 - 1) & 0x0F);
  return decodeClassicBtr(btr0, btr1, timing);
}

bool resolveClassicTiming(const SocketCANConfig& config, ClassicTiming& timing) {
  return config.useBtr ? decodeClassicBtr(config.btr0, config.btr1, timing)
                       : synthesizeClassicBtr(config.bitrateKbps, config.samplePointPercent,
                                              config.prescaler, timing);
}
...
#define FIELD(struct_type, member) \
  FieldInfo { offsetof(struct_type, member), sizeof(((struct_type*)0)->member) }
...
bool initUdpSocket(SocketHandle& sock, sockaddr_in& server, const UDPConfig& config, char* error,
...
#define SIO_UDP_CONNRESET _WSAIOW(IOC_VENDOR, 12)
  ...
  WSAIoctl(sock, SIO_UDP_CONNRESET, &disableConnReset, sizeof(disableConnReset), nullptr, 0,
...
#else
    std::snprintf(error, errorSize, "failed to set UDP socket non-blocking: %s",
                  std::strerror(errno));
#endif
    closeSocket(sock);
    sock = INVALID_SOCKET;
...
bool initTcpSocket(SocketHandle& sock, sockaddr_in& server, const TCPConfig& config, char* error,
                   std::size_t errorSize) {
#ifdef _WIN32
  static bool wsaStarted = false;
  if (!wsaStarted) {
...
#else
    std::snprintf(error, errorSize, "failed to set TCP socket non-blocking: %s",
                  std::strerror(errno));
#endif
    closeSocket(sock);
    sock = INVALID_SOCKET;
...
void forwardAssettoFrame(SPMCQueue<uint8_t, 4096>* streamBuffer, const RTCarInfo& packet) {
  ...
  for (std::size_t i = 0; i < kRTCarInfoFields.size(); ++i) {
    ...
    char frame[32]{};
    int length = std::snprintf(frame, sizeof(frame), "t%03X%1X", static_cast<unsigned>(i),
                               static_cast<unsigned>(field.size));
    const uint8_t* payload = reinterpret_cast<const uint8_t*>(base + field.offset);
    for (std::size_t b = 0;
         b < field.size && length > 0 && length < static_cast<int>(sizeof(frame) - 3); ++b)
      length += std::snprintf(frame + length, sizeof(frame) - static_cast<std::size_t>(length),
                              "%02x", payload[field.size - b - 1]);
...
bool initAssettoSocket(SocketHandle& sock, sockaddr_in& server, char* error,
                       std::size_t errorSize) {
  UDPConfig udpConfig{};
...
bool loadPcanBasic(PcanBasicApi& api, char* error, std::size_t errorSize) {
  ...
  api.initialize =
      reinterpret_cast<PcanBasicApi::InitializeFn>(GetProcAddress(api.module, "CAN_Initialize"));
  api.uninitialize = reinterpret_cast<PcanBasicApi::UninitializeFn>(
      GetProcAddress(api.module, "CAN_Uninitialize"));
  api.read = reinterpret_cast<PcanBasicApi::ReadFn>(GetProcAddress(api.module, "CAN_Read"));
  api.getValue =
      reinterpret_cast<PcanBasicApi::GetValueFn>(GetProcAddress(api.module, "CAN_GetValue"));
  api.setValue =
      reinterpret_cast<PcanBasicApi::SetValueFn>(GetProcAddress(api.module, "CAN_SetValue"));
  api.getErrorText = reinterpret_cast<PcanBasicApi::GetErrorTextFn>(
      GetProcAddress(api.module, "CAN_GetErrorText"));
  if (api.initialize && api.uninitialize && api.read && api.getValue && api.setValue &&
      api.getErrorText)
    return true;
...
TPCANBaudrate basePcanBitrate(uint32_t bitrateKbps) {
  switch (bitrateKbps) {
    ...
    case 5:
      return PCAN_BAUD_5K;
    default:
      return static_cast<TPCANBaudrate>(0);
  }
}
...
bool formatPcanError(PcanBasicApi& api, TPCANStatus status, const char* operation, char* error,
                     std::size_t errorSize) {
  ...
  std::snprintf(error, errorSize, "%s: PCAN error 0x%lX", operation,
...
bool listChannels(PcanBasicApi& api, TPCANChannelInformation* channels, DWORD& count, char* error,
                  std::size_t errorSize) {
  count = 0;
  TPCANStatus status =
      api.getValue(PCAN_NONEBUS, PCAN_ATTACHED_CHANNELS_COUNT, &count, sizeof(count));
  if (status != PCAN_ERROR_OK)
    return formatPcanError(api, status, "CAN_GetValue(PCAN_ATTACHED_CHANNELS_COUNT)", error,
                           errorSize);
  if (count == 0) return true;

  status = api.getValue(PCAN_NONEBUS, PCAN_ATTACHED_CHANNELS, channels,
                        count * sizeof(TPCANChannelInformation));
  if (status != PCAN_ERROR_OK)
    return formatPcanError(api, status, "CAN_GetValue(PCAN_ATTACHED_CHANNELS)", error, errorSize);
  return true;
}

bool resolvePcanChannel(PcanBasicApi& api, const SocketCANConfig& config, TPCANHandle& handle,
...
bool configurePcanChannel(PcanBasicApi& api, TPCANHandle handle, const SocketCANConfig& config,
                          char* error, std::size_t errorSize) {
  ...
  if (status != PCAN_ERROR_OK)
    return formatPcanError(api, status, "CAN_SetValue(PCAN_LISTEN_ONLY)", error, errorSize);
  ...
  if (status != PCAN_ERROR_OK)
    return formatPcanError(api, status, "CAN_SetValue(PCAN_BUSOFF_AUTORESET)", error, errorSize);
  return true;
}

bool forwardPcanFrame(SPMCQueue<uint8_t, 4096>* streamBuffer, const TPCANMsg& msg) {
  if ((msg.MSGTYPE & (PCAN_MESSAGE_EXTENDED | PCAN_MESSAGE_RTR | PCAN_MESSAGE_STATUS |
                      PCAN_MESSAGE_ERRFRAME)) != 0)
    return false;

  char text[32]{};
  int length =
      std::snprintf(text, sizeof(text), "t%03lX%1u", static_cast<unsigned long>(msg.ID & 0x7FFU),
                    static_cast<unsigned int>(msg.LEN));
  for (BYTE i = 0; i < msg.LEN && length > 0 && length < static_cast<int>(sizeof(text) - 3); ++i) {
    length += std::snprintf(text + length, sizeof(text) - static_cast<std::size_t>(length), "%02X",
                            msg.DATA[i]);
  }
...
void addAttr(nlmsghdr* header, std::size_t maxLength, uint16_t type, const void* data,
  ...
  auto* attr =
      reinterpret_cast<rtattr*>(reinterpret_cast<char*>(header) + NLMSG_ALIGN(header->nlmsg_len));
  attr->rta_type = type;
...
void endNest(nlmsghdr* header, rtattr* nest) {
  nest->rta_len =
      static_cast<unsigned short>(reinterpret_cast<char*>(header) + NLMSG_ALIGN(header->nlmsg_len) -
...
bool sendNetlinkAck(const nlmsghdr* request, char* error, std::size_t errorSize) {
  ...
  for (nlmsghdr* header = reinterpret_cast<nlmsghdr*>(buffer); NLMSG_OK(header, remaining);
    ...
    if (reply->error == -EPERM || reply->error == -EACCES)
      appendSocketCANPermissionsHint(error, errorSize);
    return false;
  }
...
bool configureSocketCAN(const SocketCANConfig& config, bool up, char* error,
                        std::size_t errorSize) {
  struct Request {
    nlmsghdr header;
    ifinfomsg info;
  ...
  if (request.info.ifi_index <= 0) {
    std::snprintf(error, errorSize, "if_nametoindex(%s) failed: %s", config.interfaceName,
                  std::strerror(errno));
    return false;
  }
  ...
  if (up) {
    ...
    if (timing.bitrate > 0) {
      ...
      if (timing.samplePointX10 > 0)
        bitrate.sample_point = static_cast<__u32>(timing.samplePointX10);
...
bool openSocketCAN(const SocketCANConfig& config, int& fd, char* error, std::size_t errorSize) {
  ...
  if (ioctl(fd, SIOCGIFINDEX, &request) < 0) {
    std::snprintf(error, errorSize, "ioctl(SIOCGIFINDEX, %s) failed: %s", config.interfaceName,
                  std::strerror(errno));
    close(fd);
    fd = -1;
    return false;
  ...
  if (bind(fd, reinterpret_cast<sockaddr*>(&address), sizeof(address)) < 0) {
    std::snprintf(error, errorSize, "bind(%s) failed: %s", config.interfaceName,
                  std::strerror(errno));
    if (errno == EPERM || errno == EACCES) appendSocketCANPermissionsHint(error, errorSize);
    close(fd);
    fd = -1;
...
bool forwardSocketCANFrame(SPMCQueue<uint8_t, 4096>* streamBuffer, const can_frame& frame) {
  ...
  char text[32]{};
  int length =
      std::snprintf(text, sizeof(text), "t%03X%1X", frame.can_id & CAN_SFF_MASK, frame.len);
  for (uint8_t i = 0; i < frame.len && length > 0 && length < static_cast<int>(sizeof(text) - 3);
       ++i) {
    length += std::snprintf(text + length, sizeof(text) - static_cast<std::size_t>(length), "%02X",
                            frame.data[i]);
  }
...
void Protocols::run(std::stop_token stopToken, SPMCQueue<ProtocolError, 64>* statusBuffer,
...
void Protocols::TCP(std::stop_token stopToken, SPMCQueue<ProtocolError, 64>* statusBuffer,
                    SPMCQueue<uint8_t, 4096>* streamBuffer, SPMCQueue<uint8_t, 4096>* forwardBuffer,
...
void Protocols::UDP(std::stop_token stopToken, SPMCQueue<ProtocolError, 64>* statusBuffer,
                    SPMCQueue<uint8_t, 4096>* streamBuffer, SPMCQueue<uint8_t, 4096>* forwardBuffer,
  ...
  if (config.subscribeMessage[0] != '\0') {
    const int sent = send(sock, config.subscribeMessage,
                          static_cast<int>(std::strlen(config.subscribeMessage)), 0);
...
void Protocols::UART(std::stop_token stopToken, SPMCQueue<ProtocolError, 64>* statusBuffer,
                     SPMCQueue<uint8_t, 4096>* streamBuffer,
                     SPMCQueue<uint8_t, 4096>* forwardBuffer, const UARTConfig& config) {
  (void)config;
...
void Protocols::SocketCAN(std::stop_token stopToken, SPMCQueue<ProtocolError, 64>* statusBuffer,
                          SPMCQueue<uint8_t, 4096>* streamBuffer,
...
void Protocols::AssettoCorsa(std::stop_token stopToken, SPMCQueue<ProtocolError, 64>* statusBuffer,
                             SPMCQueue<uint8_t, 4096>* streamBuffer,
  ...
  while (!stopToken.stop_requested()) {
    ...
    if (bytesRead == SOCKET_ERROR) {
      ...
      if (consecutiveReadFailures >= kMaxConsecutiveReadFailures) {
        publishStatus(statusBuffer, false,
                      "Assetto Corsa UDP read failed repeatedly; restarting reader socket");
        closeSocket(sock);
        sock = INVALID_SOCKET;
        subscribed = false;
