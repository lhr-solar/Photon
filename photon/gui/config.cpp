...
void GuiSettings::readLineFn(ImGuiContext*, ImGuiSettingsHandler*, void* entry, const char* line){
    ...
    if (sscanf(line, "Color0=%f,%f,%f,%f", &f0, &f1, &f2, &f3) == 4) { settings->colorScheme.color0 = ImVec4(f0, f1, f2, f3); return; }
    if (sscanf(line, "Color1=%f,%f,%f,%f", &f0, &f1, &f2, &f3) == 4) { settings->colorScheme.color1 = ImVec4(f0, f1, f2, f3); return; }
    if (sscanf(line, "Color2=%f,%f,%f,%f", &f0, &f1, &f2, &f3) == 4) { settings->colorScheme.color2 = ImVec4(f0, f1, f2, f3); return; }
    if (sscanf(line, "Color3=%f,%f,%f,%f", &f0, &f1, &f2, &f3) == 4) { settings->colorScheme.color3 = ImVec4(f0, f1, f2, f3); return; }
    if (sscanf(line, "Color4=%f,%f,%f,%f", &f0, &f1, &f2, &f3) == 4) { settings->colorScheme.color4 = ImVec4(f0, f1, f2, f3); return; }
    if (sscanf(line, "Color5=%f,%f,%f,%f", &f0, &f1, &f2, &f3) == 4) { settings->colorScheme.color5 = ImVec4(f0, f1, f2, f3); return; }
    if (sscanf(line, "Color6=%f,%f,%f,%f", &f0, &f1, &f2, &f3) == 4) { settings->colorScheme.color6 = ImVec4(f0, f1, f2, f3); return; }
    if (sscanf(line, "Color7=%f,%f,%f,%f", &f0, &f1, &f2, &f3) == 4) { settings->colorScheme.color7 = ImVec4(f0, f1, f2, f3); return; }
}
...
void GuiSettings::writeAllFn(ImGuiContext* ctx, ImGuiSettingsHandler* handler, ImGuiTextBuffer* out_buf){
    ...
    out_buf->appendf("ColorSeed=%.6f,%.6f,%.6f,%.6f\n",
        settings->colorSeed.x, settings->colorSeed.y, settings->colorSeed.z, settings->colorSeed.w);
    out_buf->appendf("Color0=%.6f,%.6f,%.6f,%.6f\n", settings->colorScheme.color0.x, settings->colorScheme.color0.y, settings->colorScheme.color0.z, settings->colorScheme.color0.w);
    out_buf->appendf("Color1=%.6f,%.6f,%.6f,%.6f\n", settings->colorScheme.color1.x, settings->colorScheme.color1.y, settings->colorScheme.color1.z, settings->colorScheme.color1.w);
    out_buf->appendf("Color2=%.6f,%.6f,%.6f,%.6f\n", settings->colorScheme.color2.x, settings->colorScheme.color2.y, settings->colorScheme.color2.z, settings->colorScheme.color2.w);
    out_buf->appendf("Color3=%.6f,%.6f,%.6f,%.6f\n", settings->colorScheme.color3.x, settings->colorScheme.color3.y, settings->colorScheme.color3.z, settings->colorScheme.color3.w);
    out_buf->appendf("Color4=%.6f,%.6f,%.6f,%.6f\n", settings->colorScheme.color4.x, settings->colorScheme.color4.y, settings->colorScheme.color4.z, settings->colorScheme.color4.w);
    out_buf->appendf("Color5=%.6f,%.6f,%.6f,%.6f\n", settings->colorScheme.color5.x, settings->colorScheme.color5.y, settings->colorScheme.color5.z, settings->colorScheme.color5.w);
    out_buf->appendf("Color6=%.6f,%.6f,%.6f,%.6f\n", settings->colorScheme.color6.x, settings->colorScheme.color6.y, settings->colorScheme.color6.z, settings->colorScheme.color6.w);
    out_buf->appendf("Color7=%.6f,%.6f,%.6f,%.6f\n", settings->colorScheme.color7.x, settings->colorScheme.color7.y, settings->colorScheme.color7.z, settings->colorScheme.color7.w);
    out_buf->append("\n");
    ...
    out_buf->appendf("FillColor=%.6f,%.6f,%.6f,%.6f\n",
        settings->plotLineSpec.FillColor.x, settings->plotLineSpec.FillColor.y,
        settings->plotLineSpec.FillColor.z, settings->plotLineSpec.FillColor.w);
    ...
    out_buf->appendf("MarkerLineColor=%.6f,%.6f,%.6f,%.6f\n",
        settings->plotLineSpec.MarkerLineColor.x, settings->plotLineSpec.MarkerLineColor.y,
        settings->plotLineSpec.MarkerLineColor.z, settings->plotLineSpec.MarkerLineColor.w);
    out_buf->appendf("MarkerFillColor=%.6f,%.6f,%.6f,%.6f\n",
        settings->plotLineSpec.MarkerFillColor.x, settings->plotLineSpec.MarkerFillColor.y,
        settings->plotLineSpec.MarkerFillColor.z, settings->plotLineSpec.MarkerFillColor.w);
...
void GuiSettings::colorUI() {
    ...
    if (ImGui::BeginPopupModal("Theme", nullptr, flags)) {
        ...
        if (ImGui::Button("-")) { plotLineSpec.LineWeight = std::max(1.0f, plotLineSpec.LineWeight - 1.0f); dirty = true; }
        ImGui::SameLine();
