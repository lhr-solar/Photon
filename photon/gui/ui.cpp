...
void UI::showRenderedFrames()
{
    ...
    if (ImGui::Begin("Rendered 3D Frame"))
    {
        ...
        if (renderedFrameTexture != static_cast<ImTextureID>(0))
        {
            ImVec2 imageSize = ImVec2(800.0f, 600.0f);

            ImGui::Image(renderedFrameTexture, imageSize);
        }
        else
        {
            // Placeholder if texture is not yet available
            ImGui::TextColored(ImVec4(1.0f, 1.0f, 0.0f, 1.0f), "Waiting for rendered frame...");
            ImGui::TextWrapped("The 3D GLTF model is being rendered to the swap chain.");
            ImGui::TextWrapped("The frame will appear here once the texture is initialized.");
        }
