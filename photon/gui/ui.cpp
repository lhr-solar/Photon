...
static Video gVideoPlayer;

void UI::showVideoWindow() {
    ImGui::Begin("Video Player");
    // Initialize and open video if not already done
    static bool videoLoaded = false;
    if (!videoLoaded) {
        if (gVideoPlayer.open("../mp4Videos/F1POV.mp4")) { // << removed extra paren
            videoLoaded = true;
        } else {
            ImGui::Text("Failed to open video");
            ImGui::End();
            return;
        }
    }
    // Advance frame (call once per frame)
    gVideoPlayer.updateFrame();
    // Draw the current frame (assume frame exposes a texture or raw pixels)
    const frame& current = gVideoPlayer.getFrame();
    if (current.isValid()) { // << use isValid(), NOT hasData()
        // TODO: Upload current.data as a Vulkan texture, get ImTextureID
        // Example placeholder:
        // ImTextureID imguiTexHandle = ...; // Your texture upload logic here
        // ImGui::Image(imguiTexHandle, ImVec2(gVideoPlayer.width(), gVideoPlayer.height()));
        ImGui::Text("Frame loaded, texture upload/display not yet implemented.");
    } else {
