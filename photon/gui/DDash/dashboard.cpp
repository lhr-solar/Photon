  ...
  switch (bpsFaultCode) {
    case 1:
      if (extrema.maxVoltIdx >= 0) {
        snprintf(buffer, bufferSize, "BPS: Mod %d (%.2fV) OVERVOLT", extrema.maxVoltIdx,
                 extrema.maxVolt);
        return true;
      }
      break;
    case 2:
      if (extrema.minVoltIdx >= 0) {
        snprintf(buffer, bufferSize, "BPS: Mod %d (%.2fV) UNDERVOLT", extrema.minVoltIdx,
                 extrema.minVolt);
        return true;
      }
      break;
    case 4:
      if (extrema.maxTempIdx >= 0) {
        snprintf(buffer, bufferSize, "BPS: Mod %d (%.1fC) OVERTEMP", extrema.maxTempIdx,
                 extrema.maxTemp);
        return true;
      }
      break;
...
static bool FormatBpsExtremaSummary(const AppState& state, char* buffer, size_t bufferSize) {
  ...
  if (extrema.minVoltIdx >= 0)
    snprintf(lowBuf, sizeof(lowBuf), lowFmt, extrema.minVoltIdx, extrema.minVolt);
  else
    snprintf(lowBuf, sizeof(lowBuf), "%s", lowFmt);

  if (extrema.maxVoltIdx >= 0)
    snprintf(highBuf, sizeof(highBuf), highFmt, extrema.maxVoltIdx, extrema.maxVolt);
  else
    snprintf(highBuf, sizeof(highBuf), "%s", highFmt);

  if (extrema.maxTempIdx >= 0)
    snprintf(hotBuf, sizeof(hotBuf), hotFmt, extrema.maxTempIdx, extrema.maxTemp);
  else
    snprintf(hotBuf, sizeof(hotBuf), "%s", hotFmt);
...
static void RenderFaultBanner(const AppState& state, const ImVec2& boundsMin,
  ...
  for (const ImVec2& off : kBoldOffsets) {
    dl->AddText(bigFont, fontSize, ImVec2(textOrigin.x + off.x, textOrigin.y + off.y),
                knockoutColor, text);
  }
}
...
static void RenderCameraView(const AppState& state, const char* label, void* texture,
                             const ImVec2& size) {
...
static void RenderBatteryPanel(const AppState& state, const ImVec2& size) {
  ...
  ImGui::BeginChild("##BatteryPanel", size, ImGuiChildFlags_None, ImGuiWindowFlags_NoScrollbar);
  {
...

    {
      ...
      ImGuiIO& io = ImGui::GetIO();
      ImFont* medFont = (io.Fonts->Fonts.Size > 2) ? io.Fonts->Fonts[2] : nullptr;
      float fs = 28.0f;
      if (medFont) {
        ImVec2 sz = medFont->CalcTextSizeA(fs, FLT_MAX, 0, currentLabel);
        ImVec2 pos = ImGui::GetCursorScreenPos();
        float centeredX = pos.x + (avW - sz.x) * 0.5f;
        dl->AddText(medFont, fs, ImVec2(centeredX, pos.y), ColorToU32(Colors::Foreground()),
                    currentLabel);
    ...
    auto drawBatteryRow = [&](const char* rowLabel, float soc, float voltage,
      ...
      // SOC bar with percentage overlay
      {
        ...
        dl->AddRectFilled(barPos, ImVec2(barPos.x + barW, barPos.y + barH), ColorToU32(bgCol),
                          rounding);
        // Fill
        ...
        if (fillW > 0.0f) {
          ...
          dl->AddRectFilled(barPos, ImVec2(barPos.x + fillW, barPos.y + barH), ColorToU32(batColor),
                            fillR, fillFlags);
        }

        dl->AddRect(barPos, ImVec2(barPos.x + barW, barPos.y + barH),
                    ColorToU32(ColorWithAlpha(Colors::MutedForeground(), 0.42f)), rounding, 0,
                    1.0f);

        char socTxt[8];
        snprintf(socTxt, sizeof(socTxt), "%.0f%%", boundedSoc);
        ImVec2 socSz = ImGui::CalcTextSize(socTxt);
        ImVec2 socPos(barPos.x + (barW - socSz.x) * 0.5f, barPos.y + (barH - socSz.y) * 0.5f);
...
static void RenderButtonGrid(AppState& state, const ImVec2& size) {
  ...
  ImGui::BeginChild("##ButtonGrid", size, ImGuiChildFlags_None, ImGuiWindowFlags_NoScrollbar);
  {
    struct BtnDef {
      const char* label;
      bool* statePtr;
    ...
    BtnDef contactors[3][2] = {
        {{"HV+", &hvPositive}, {"HV-", &hvNegative}},
        {{"Array Pre", &arrayPrecharge}, {"Array", &arrayContactor}},
        {{"Motor Pre", &motorPrecharge}, {"Motor", &motorContactor}},
    };
    ...
    ImDrawList* dl = ImGui::GetWindowDrawList();
    ImGuiIO& io = ImGui::GetIO();
    ImFont* labelFont = (io.Fonts->Fonts.Size > 2) ? io.Fonts->Fonts[2] : nullptr;
    ...
    auto drawButton = [&](const BtnDef& b, float bw, const char* idSuffix) {
      ...
      ImVec4 textCol = active ? Colors::PrimaryForeground() : Colors::MutedForeground();
      ImVec4 outlineCol = active ? ColorWithAlpha(Colors::PrimaryForeground(), 0.45f)
      ...
      ImVec2 tSz = labelFont ? labelFont->CalcTextSizeA(fs, FLT_MAX, 0, b.label)
                             : ImGui::CalcTextSize(b.label);
      ImVec2 center(cellPos.x + bw * 0.5f, cellPos.y + btnH * 0.5f);
      dl->AddText(labelFont, fs, ImVec2(center.x - tSz.x * 0.5f, center.y - tSz.y * 0.5f),
    ...
    ImGui::Dummy(ImVec2(0, dividerH * 0.5f));
    {
      ...
      dl->AddLine(ImVec2(pos.x + inset, pos.y), ImVec2(pos.x + avail.x - inset, pos.y),
...
static void RenderSpeedGauge(AppState& state, const ImVec2& size) {
  ...
  ImGui::BeginChild("##SpeedGauge", size, ImGuiChildFlags_None, ImGuiWindowFlags_NoScrollbar);
  {
    ImDrawList* dl = ImGui::GetWindowDrawList();
    ...
    //  hb
    {
      ...
      icons::DrawHeart(dl, ImVec2(origin.x + 28.0f, origin.y + 22.0f), 30.0f, ColorToU32(hbCol));
    }
    ...
    if (pct > 0.001f) {
      ...
      dl->PathArcTo(center, radius, startAngle, startAngle + sweepAngle * pct, 64);
    ...
    // FNR gear display below icons
    {
      ...
      // Pedal Percentage Bar (inside FNR scope to access fnrY/fnrH)
      {
        ...
        dl->AddText(medFont, pressureFs, ImVec2(cX - pSz.x * 0.5f, pressureY),
                    ColorToU32(pressureCol), pressureTxt);
      }
    }
  }
  ImGui::EndChild();
...
static const char* BpsFaultName(uint8_t code) {
  switch (code) {
    ...
    case 9:
      return "HV- Ctr Sense";
    case 10:
      return "Array Ctr Sense";
    case 11:
      return "Array Pchg Ctr Sense";
    case 12:
      return "Estop 1";
    case 13:
      return "Estop 2";
  ...
  if (ImGui::BeginTable(
    ...
    for (int i = 0; i < 32; i++) {
      ...
      if (i < nMod) {
        ...
        if (v > 4.2f || v < 2.5f)
          vCol = Colors::Destructive();
        else if (v > 4.15f || v < 3.0f)
          vCol = Colors::Warning();

        ImVec4 tCol = Colors::Foreground();
        if (t > 45.0f)
          tCol = Colors::Destructive();
        else if (t > 35.0f)
          tCol = Colors::Warning();
...
static void RenderDebugScreen(AppState& liveState) {
  ...
  const AppState& state =
      (g_selectedSnapshotIndex >= 0 && g_selectedSnapshotIndex < g_faultSnapshots.size())
  ...
  if (ImGui::BeginCombo("##Snapshots",
                        g_selectedSnapshotIndex == -1
                            ? "[LIVE DATA]"
                            : g_faultSnapshots[g_selectedSnapshotIndex].first.c_str())) {
    if (ImGui::Selectable("[LIVE DATA]", g_selectedSnapshotIndex == -1))
      g_selectedSnapshotIndex = -1;
  ...
  // ===== LEFT COLUMN (INPUTS) =====
  ImGui::BeginChild("DebugL", ImVec2(colW, avail.y - 70), ImGuiChildFlags_None,
  ...
  ImGui::Text(
      " Horn:%s Hazard:%s PTT:%s Cruise Set:%s Regen Act:%s",
  ...
  ImGui::Text(" Gear: %s  Turn: %s", GearToString(state.gear),
              state.turnSignal == TurnSignal::Left    ? "L"
              : state.turnSignal == TurnSignal::Right ? "R"
                                                      : "-");
  widgets::Space(4);

  // Accel/Brake Pedals
  ImGui::TextColored(ImVec4(0.8f, 0.6f, 1.0f, 1.0f), "ACCEL / BRAKE PEDALS");
  ImGui::Text(" Accel: Total %.0f%%  Main %d%% Red %d%% V:%.2f/%.2f",
              state.get("AccelPedal_Main_Pos"), (uint8_t)state.get("AccelPedal_Main_Pos"),
              (uint8_t)state.get("AccelPedal_Redundant_Pos"), state.get("Accel_Pos_Voltage_Main"),
              state.get("Accel_Pos_Voltage_Redundant"));
  ImGui::Text("        Faults: Main:%s Red:%s", flt(state.getBool("AccelPedal_Main_Fault")),
              flt(state.getBool("AccelPedal_Redundant_Fault")));
  ImGui::Text(" Brake: Engaged:%s  Main %d%% Red %d%% V:%.2f/%.2f",
              state.brakeEngaged ? "YES" : "NO", (uint8_t)state.get("BrakePedal_Main_Pos"),
              (uint8_t)state.get("BrakePedal_Redundant_Pos"), state.get("Brake_Pos_Voltage_Main"),
              state.get("Brake_Pos_Voltage_Redundant"));
  ImGui::Text("        Faults: Main:%s Red:%s", flt(state.getBool("BrakePedal_Main_Fault")),
              flt(state.getBool("BrakePedal_Redundant_Fault")));
  ImGui::Text(" Pressure: %.0f / %.0f psi (%s/%s)", state.get("Brake_Pressure_1"),
              state.get("Brake_Pressure_2"), flt(state.getBool("Brake_Pressure_1_Fault")),
              flt(state.getBool("Brake_Pressure_2_Fault")));
  widgets::Space(4);
  ...
  ImGui::Text(" HV+:%s -:%s ArrP:%s Arr:%s MtrP:%s Mtr:%s",
              state.getBool("HV_Plus_Contactor_State") ? "C" : "o",
              state.getBool("HV_Minus_Contactor_State") ? "C" : "o",
              state.getBool("Array_Precharge_Contactor_State") ? "C" : "o",
              state.getBool("Array_Contactor_State") ? "C" : "o",
              state.getBool("Motor_Precharge_Contactor_State") ? "C" : "o",
              state.getBool("Motor_Contactor_State") ? "C" : "o");
  ImGui::Text(
      " Ignition: LV:%s  Array:%s  Motor:%s",
  ...
  ImGui::SameLine();
  ImGui::BeginChild("DebugR", ImVec2(colW, avail.y - 70), ImGuiChildFlags_None,
                    ImGuiWindowFlags_None);
  ImGui::SetWindowFontScale(1.3f);
  ...
  ImGui::Text(" Vel: %d mph  Bus: %.1fV %.1fA", state.speed, state.get("MC_BusVoltage"),
              state.get("MC_BusCurrent"));
  ImGui::Text(" Phase: %.1fA (B) %.1fA (C)  BEMF: %.1fV (Q) %.1fV (D)",
              state.get("MC_PhaseCurrentB"), state.get("MC_PhaseCurrentC"), state.get("MC_BEMFq"),
              state.get("MC_BEMFd"));
  ImGui::Text(" Temp: Heatsink %.1fC  Precharge Motor: %.1fV", state.get("MC_HeatsinkTemp"),
              state.get("VCU_Precharge_Motor_Voltage"));
  ImGui::TextColored(
      Colors::Warning(), " Limits Active: %s%s%s%s%s%s%s%s",
      limitMotorCurrent ? "MotorCurrent " : "", limitVelocity ? "Velocity " : "",
      limitBusCurrent ? "BusCurrent " : "", limitBusVoltageUpper ? "BusVolUpper " : "",
      limitBusVoltageLower ? "BusVolLower " : "", limitIpmOrMotorTemp ? "Temp " : "",
      limitOutputVoltage ? "OutVolPWM " : "",
      (!limitMotorCurrent && !limitVelocity && !limitBusCurrent && !limitBusVoltageUpper &&
       !limitBusVoltageLower && !limitIpmOrMotorTemp && !limitOutputVoltage)
          ? "None"
          : "");
  widgets::Space(4);
  ...
  const uint8_t vcuFaultCode = (uint8_t)state.get("VCU_Fault");
  const bool vcuPedalsOK =
      state.signals.count("VCU_Pedals_Watchdog") && state.get("VCU_Pedals_Watchdog") == 0.0;
  const bool vcuDriverInputOK = state.signals.count("VCU_Driver_Input_Watchdog") &&
                                state.get("VCU_Driver_Input_Watchdog") == 0.0;
  ...
  ImGui::TextColored(vcuFCol, " Fault: %d (%s)  FSM State: %d", vcuFaultCode,
                     VcuFaultName(vcuFaultCode), (uint8_t)state.get("VCU_FSM_State"));
  ImGui::Text(" Motor Ready:%s Pedals:%s Driver Input:%s", state.getBool("Motor_Ready") ? "Y" : "N",
              vcuPedalsOK ? "OK" : "NO", vcuDriverInputOK ? "OK" : "NO");
  ImGui::Text(" Regen:%s (Active:%s)", state.getBool("VCU_Regen_OK") ? "OK" : "NO",
              state.getBool("VCU_Regen_Active") ? "Y" : "N");
  widgets::Space(4);
  ...
  ImGui::Text(" In: %.1fV %.2fA (%.0fW)  Out: %.1fV %.2fA (%.0fW)", mpptVin, mpptIin, mpptPIn,
              mpptVout, mpptIout, mpptPOut);
  ...
  ImGui::Text(" Coolant: %.1f / %.1fC  Flow: %.2f / %.2f L/min", state.get("Coolant_Temperature_1"),
              state.get("Coolant_Temperature_2"), state.get("FlowRate_1"), state.get("FlowRate_2"));
  ImGui::TextColored(pumpCol, " Pump: %d%% %s", (uint8_t)state.get("Pump_DutyCycle"),
                     pumpFault ? "FAULT" : "OK");
  widgets::Space(4);
  ...
  ImGui::Text(" Supp Batt Info: SOC: %.1f%%  %.1fV  %.1fA  Charger: %s  DCDC: %.1fV %.1fA",
              ...
              state.get("Supplemental_Battery_Current") * 0.001,
              SuppChargerStatusStr((uint8_t)state.get("SuppCharger_Status")),
              state.get("Supplemental_DCDC_Voltage"),
              state.get("Supplemental_DCDC_Current") * 0.001);
  ...
  ImGui::TextColored(bpsFCol, " Fault: %d (%s)  Regen: %s  Charge: %s", bpsFaultCode,
                     BpsFaultName(bpsFaultCode), state.getBool("BPS_Regen_OK") ? "OK" : "NO",
                     state.getBool("BPS_Charge_OK") ? "OK" : "NO");
  const float mainBatteryAvgTemp = (float)state.get("Main_Battery_Avg_Temperature");
  if (mainBatteryAvgTemp > 0.0f) {
...
void RenderDashboard(AppState& state, ImGuiWindowFlags flags) {
  ...
  if (!state.showDebugScreen) {  // Only record edge-triggered snapshot when not actively browsing
                                 // snapshots
  ...
  // on top later in the frame. Single-pointer-friendly (works on touch).
  {
    ...
    if (ImGui::IsMouseClicked(ImGuiMouseButton_Left)) {
      ...
      if (mp.y >= windowPos.y && mp.y <= windowPos.y + CORNER_PX) {
        ...
        if (mp.x >= windowPos.x + windowSize.x - CORNER_PX && mp.x <= windowPos.x + windowSize.x) {
          lastRightClickT = nowT;
        }
      }
      if ((nowT - lastLeftClickT) < CHORD_WINDOW_S && (nowT - lastRightClickT) < CHORD_WINDOW_S) {
  ...
  if (state.showDebugScreen) {
    ...
    RenderCameraView(state, "LEFT VIEW", state.leftCameraTexture, ImVec2(topColLeft, rowTop));
    ...
    RenderCameraView(state, "RIGHT VIEW", state.rightCameraTexture, ImVec2(topColRight, rowTop));
    ...
    RenderCameraView(state, "REAR VIEW", state.rearCameraTexture, ImVec2(rearBotW, rowBottom));
