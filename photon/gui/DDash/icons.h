...
inline void DrawCruiseControl(ImDrawList* dl, ImVec2 center, float size, ImU32 color) {
  ...
  for (int i = 0; i <= 8; i++) {
    ...
    dl->AddLine(ImVec2(center.x + cosf(a) * innerR, center.y + sinf(a) * innerR),
                ImVec2(center.x + cosf(a) * outerR, center.y + sinf(a) * outerR), color, thick);
  }
...
inline void DrawRegen(ImDrawList* dl, ImVec2 center, float size, ImU32 color) {
  ...
  for (int i = 0; i < 3; i++) {
    ...
    ImVec2 tip(tipX + cosf(tipAngle) * headSize, tipY + sinf(tipAngle) * headSize);
    ImVec2 left(tipX + cosf(perpAngle) * headSize * 0.5f, tipY + sinf(perpAngle) * headSize * 0.5f);
