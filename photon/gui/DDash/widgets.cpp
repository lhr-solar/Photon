...
bool BeginCard(const char* id, const ImVec2& size, bool border) {
  ...
  ImGui::PushStyleVar(ImGuiStyleVar_WindowPadding,
                      ImVec2(Spacing::CardPadding, Spacing::CardPadding));
...
bool LabeledToggle(const char* label, bool* value) {
  ...
  drawList->AddCircleFilled(ImVec2(knobX, pos.y + radius), radius - 3.0f,
                            ColorToU32(Colors::Foreground()));
...
void CircularProgress(const char* label, float value, float radius, float thickness,
  ...
  // Background arc
  drawList->PathArcTo(center, radius - thickness * 0.5f, startAngle, startAngle + maxAngle,
                      numSegments);
  ...
  if (value > 0.001f) {
    ...
    drawList->PathArcTo(center, radius - thickness * 0.5f, startAngle, startAngle + progressAngle,
                        numSegments);
...
void Sparkline(const float* values, int count, float minVal, float maxVal, const ImVec2& size,
               const ImVec4& color) {
...
void LineChart(const char* label, const float* values, int count, float minVal, float maxVal,
  ...
  for (int i = 0; i < count - 1; i++) {
    float x1 =
        pos.x + padding + (static_cast<float>(i) / static_cast<float>(count - 1)) * chartWidth;
    float x2 =
        pos.x + padding + (static_cast<float>(i + 1) / static_cast<float>(count - 1)) * chartWidth;
...
void BarChart(const char* label, const float* values, int count, const char** labels,
              const ImVec2& size, const ImVec4& color) {
...
void ValueDisplay(const char* label, const char* value, const char* unit,
...
void Space(float height) { ImGui::Dummy(ImVec2(0, height)); }
