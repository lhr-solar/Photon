...
void gpuGUI::buildUI(GPU& gpu){
    ...
    if(ImGui::Begin("GPU Info Window", nullptr, flags)){
        ...
        for(auto i{0uz}; i < heapCount; i++){
            ...
            ImGui::Text("Heap Index %i Usage: %s / %s", static_cast<int>(i), usedSize.c_str(), size.c_str()); ImGui::SameLine();
            for(const auto& s : v){
                ImGui::Text("%s", s.c_str());
                ImGui::SameLine();
            }
            ...
            for(auto j{0uz}; j < memTypeCount; ++j){
                ...
                ImGui::Indent();
                ImGui::Text("Memory Type %i:", static_cast<int>(j)); ImGui::SameLine();
        ...
        ImGui::Text("Buffer Count: %i Total Size: %s", static_cast<int>(gpu.bufferHandles.size()), fmtb(totalBufferBytes).c_str());
        for (auto i{0uz}; i < gpu.bufferHandles.size(); ++i) {
            ImGui::Text("Buffer %i Size: %s", static_cast<int>(i), fmtb(gpu.bufferSizes[i]).c_str()); ImGui::SameLine();
        ...
        for (auto i{0uz}; i < gpu.imageHandles.size(); ++i) {
            ...
            ImGui::Text("Image %i: %ux%ux%u %s %s",
                static_cast<int>(i),
                extent.width,
                extent.height,
                extent.depth,
                VkFormatToString(gpu.imageFormats[i]),
                VkSampleCountFlagBitsToString(gpu.imageSampleCounts[i])); ImGui::SameLine();
        ...
        for (auto i{0uz}; i < gpu.descriptorPoolHandles.size(); ++i) {
            ImGui::Text("Descriptor Pool %i Max Sets: %u", static_cast<int>(i), gpu.descriptorPoolMaxSets[i]); ImGui::SameLine();
        ...
        ImGui::Text("Descriptor Set Layout Count: %i", static_cast<int>(gpu.descriptorSetLayoutHandles.size()));
        for (auto i{0uz}; i < gpu.descriptorSetLayoutHandles.size(); ++i) {
            ImGui::Text("Descriptor Set Layout %i Binding Count: %u", static_cast<int>(i), gpu.descriptorSetLayoutBindingCounts[i]);
        }
        ImGui::Separator();
        ...
        for (auto i{0uz}; i < gpu.descriptorSetHandles.size(); ++i) {
            ImGui::Text("Descriptor Set %i Pool: %s Layout: %s",
                static_cast<int>(i),
        ...
        for (auto i{0uz}; i < gpu.commandPoolHandles.size(); ++i) {
            ImGui::Text("Command Pool %i Queue Family: %u", static_cast<int>(i), gpu.commandPoolQueueFamilyIndices[i]); ImGui::SameLine();
        ...
        for (auto i{0uz}; i < gpu.commandBufferHandles.size(); ++i) {
            ImGui::Text("Command Buffer %i Pool: %s Level: %s",
                static_cast<int>(i),
...
std::vector<std::string> gpuGUI::VkMemoryPropertyFlagsToString(VkMemoryPropertyFlags flags){
    ...
    if (flags & VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT) names.emplace_back("VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT");
    if (flags & VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT) names.emplace_back("VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT");
    if (flags & VK_MEMORY_PROPERTY_HOST_COHERENT_BIT) names.emplace_back("VK_MEMORY_PROPERTY_HOST_COHERENT_BIT");
    if (flags & VK_MEMORY_PROPERTY_HOST_CACHED_BIT) names.emplace_back("VK_MEMORY_PROPERTY_HOST_CACHED_BIT");
    if (flags & VK_MEMORY_PROPERTY_LAZILY_ALLOCATED_BIT) names.emplace_back("VK_MEMORY_PROPERTY_LAZILY_ALLOCATED_BIT");
    if (flags & VK_MEMORY_PROPERTY_PROTECTED_BIT) names.emplace_back("VK_MEMORY_PROPERTY_PROTECTED_BIT");
    if (flags & VK_MEMORY_PROPERTY_DEVICE_COHERENT_BIT_AMD) names.emplace_back("VK_MEMORY_PROPERTY_DEVICE_COHERENT_BIT_AMD");
    if (flags & VK_MEMORY_PROPERTY_DEVICE_UNCACHED_BIT_AMD) names.emplace_back("VK_MEMORY_PROPERTY_DEVICE_UNCACHED_BIT_AMD");
    if (flags & VK_MEMORY_PROPERTY_RDMA_CAPABLE_BIT_NV) names.emplace_back("VK_MEMORY_PROPERTY_RDMA_CAPABLE_BIT_NV");
    return names;
}

std::vector<std::string> gpuGUI::VkMemoryHeapFlagsToString(VkMemoryHeapFlags flags){
    std::vector<std::string> names{};
    if (flags & VK_MEMORY_HEAP_DEVICE_LOCAL_BIT) names.emplace_back("VK_MEMORY_HEAP_DEVICE_LOCAL_BIT");
    if (flags & VK_MEMORY_HEAP_MULTI_INSTANCE_BIT) names.emplace_back("VK_MEMORY_HEAP_MULTI_INSTANCE_BIT");
    return names;
}
...
std::vector<std::string> gpuGUI::VkBufferUsageFlagsToString(VkBufferUsageFlags flags){
    std::vector<std::string> names{};
    if (flags & VK_BUFFER_USAGE_TRANSFER_SRC_BIT) names.emplace_back("VK_BUFFER_USAGE_TRANSFER_SRC_BIT");
    if (flags & VK_BUFFER_USAGE_TRANSFER_DST_BIT) names.emplace_back("VK_BUFFER_USAGE_TRANSFER_DST_BIT");
    if (flags & VK_BUFFER_USAGE_UNIFORM_TEXEL_BUFFER_BIT) names.emplace_back("VK_BUFFER_USAGE_UNIFORM_TEXEL_BUFFER_BIT");
    if (flags & VK_BUFFER_USAGE_STORAGE_TEXEL_BUFFER_BIT) names.emplace_back("VK_BUFFER_USAGE_STORAGE_TEXEL_BUFFER_BIT");
    if (flags & VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT) names.emplace_back("VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT");
    if (flags & VK_BUFFER_USAGE_STORAGE_BUFFER_BIT) names.emplace_back("VK_BUFFER_USAGE_STORAGE_BUFFER_BIT");
    if (flags & VK_BUFFER_USAGE_INDEX_BUFFER_BIT) names.emplace_back("VK_BUFFER_USAGE_INDEX_BUFFER_BIT");
    if (flags & VK_BUFFER_USAGE_VERTEX_BUFFER_BIT) names.emplace_back("VK_BUFFER_USAGE_VERTEX_BUFFER_BIT");
    if (flags & VK_BUFFER_USAGE_INDIRECT_BUFFER_BIT) names.emplace_back("VK_BUFFER_USAGE_INDIRECT_BUFFER_BIT");
    return names;
}

std::vector<std::string> gpuGUI::VkImageUsageFlagsToString(VkImageUsageFlags flags){
    std::vector<std::string> names{};
    if (flags & VK_IMAGE_USAGE_TRANSFER_SRC_BIT) names.emplace_back("VK_IMAGE_USAGE_TRANSFER_SRC_BIT");
    if (flags & VK_IMAGE_USAGE_TRANSFER_DST_BIT) names.emplace_back("VK_IMAGE_USAGE_TRANSFER_DST_BIT");
    ...
    if (flags & VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT) names.emplace_back("VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT");
    if (flags & VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT) names.emplace_back("VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT");
    if (flags & VK_IMAGE_USAGE_TRANSIENT_ATTACHMENT_BIT) names.emplace_back("VK_IMAGE_USAGE_TRANSIENT_ATTACHMENT_BIT");
    if (flags & VK_IMAGE_USAGE_INPUT_ATTACHMENT_BIT) names.emplace_back("VK_IMAGE_USAGE_INPUT_ATTACHMENT_BIT");
    return names;
}

std::vector<std::string> gpuGUI::VkDescriptorPoolCreateFlagsToString(VkDescriptorPoolCreateFlags flags){
    std::vector<std::string> names{};
    if (flags == 0) names.emplace_back("VK_DESCRIPTOR_POOL_CREATE_NONE");
    if (flags & VK_DESCRIPTOR_POOL_CREATE_FREE_DESCRIPTOR_SET_BIT) names.emplace_back("VK_DESCRIPTOR_POOL_CREATE_FREE_DESCRIPTOR_SET_BIT");
    if (flags & VK_DESCRIPTOR_POOL_CREATE_UPDATE_AFTER_BIND_BIT) names.emplace_back("VK_DESCRIPTOR_POOL_CREATE_UPDATE_AFTER_BIND_BIT");
    if (flags & VK_DESCRIPTOR_POOL_CREATE_HOST_ONLY_BIT_EXT) names.emplace_back("VK_DESCRIPTOR_POOL_CREATE_HOST_ONLY_BIT_EXT");
    return names;
}
...
std::vector<std::string> gpuGUI::VkCommandPoolCreateFlagsToString(VkCommandPoolCreateFlags flags){
    ...
    if (flags & VK_COMMAND_POOL_CREATE_TRANSIENT_BIT) names.emplace_back("VK_COMMAND_POOL_CREATE_TRANSIENT_BIT");
    if (flags & VK_COMMAND_POOL_CREATE_RESET_COMMAND_BUFFER_BIT) names.emplace_back("VK_COMMAND_POOL_CREATE_RESET_COMMAND_BUFFER_BIT");
    if (flags & VK_COMMAND_POOL_CREATE_PROTECTED_BIT) names.emplace_back("VK_COMMAND_POOL_CREATE_PROTECTED_BIT");
    return names;
}
...
const char* gpuGUI::VkSampleCountFlagBitsToString(VkSampleCountFlagBits flags){
    switch (flags) {
        ...
        default: return "VK_SAMPLE_COUNT_UNKNOWN";
    }
}
...
const char* gpuGUI::VkCommandBufferLevelToString(VkCommandBufferLevel level){
    switch (level) {
        ...
        default: return "VK_COMMAND_BUFFER_LEVEL_UNKNOWN";
    }
}
