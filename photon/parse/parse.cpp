...
void buildConfig(std::istream& stream, arenaConfig& config){
    ...
    while(std::getline(stream, line)){
        ...
        if(line.rfind("BO_", 0) == 0){
            ...
            try{ dlc = static_cast<uint8_t>(std::stoi(dlcStr)); } catch (...) { continue; }
            (void)dlc;
            haveMsg = true;
            currentId = canId;
...
void populateArena(Arena& arena, std::istream& stream){
    ...
    while(std::getline(stream, line)){
        ...
        if(line.rfind("BO_", 0) == 0){
            ...
            try{ rawType = static_cast<uint32_t>(std::stoi(typeStr)); } catch (...) { continue; }
...
const char* Parse::currentDBCName() const{
    return activeDBCLabel.c_str();
}
