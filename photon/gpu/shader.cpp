...
static void destroyFrame(Shader& shader, uint32_t index) {
  ...
  if (frame.framebuffer != VK_NULL_HANDLE)
    vkDestroyFramebuffer(shader.device, frame.framebuffer, NULL);
...
static void initFrame(Shader& shader, GPU& gpu, uint32_t index) {
  ...
  imageAllocInfo.memoryTypeIndex =
      gpu.getMemoryType(memReqs.memoryTypeBits, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);
...
void Shader::init(GPU& gpu, uint32_t* vertexShader, size_t vertexShaderSize,
...
void Shader::prepareInit(GPU& gpu, uint32_t* vertexShader, size_t vertexShaderSize,
                         uint32_t* fragmentShader, size_t fragmentShaderSize) {
  this->gpu = &gpu;
  device = gpu.device;
...
void Shader::dispatchInit(GPU& gpu, uint32_t* vertexShader, size_t vertexShaderSize,
                          uint32_t* fragmentShader, size_t fragmentShaderSize) {
  gpuAsyncDispatches.fetch_add(1, std::memory_order_relaxed);
  const std::vector<uint32_t> vertexCopy =
      vertexShader != nullptr && vertexShaderSize != 0
  ...
  const std::vector<uint32_t> fragmentCopy =
      fragmentShader != nullptr && fragmentShaderSize != 0
          ...
          : std::vector<uint32_t>{};
  std::thread([this, &gpu, vertexCopy = std::move(vertexCopy), vertexShaderSize,
               fragmentCopy = std::move(fragmentCopy), fragmentShaderSize]() {
    tracy::SetThreadName("Shader Init");
    AsyncDispatchGuard guard{};
    prepareInit(gpu, vertexCopy.empty() ? nullptr : const_cast<uint32_t*>(vertexCopy.data()),
                vertexShaderSize,
                fragmentCopy.empty() ? nullptr : const_cast<uint32_t*>(fragmentCopy.data()),
                fragmentShaderSize);
  }).detach();
}
...
void Shader::render(GPU& gpu, VkCommandBuffer& commandBuffer) {
  ...
  gpu.setImageLayout(commandBuffer, frame.image, oldLayout,
                     VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL, range, srcStage,
  ...
  pc.resolution =
      glm::vec2(static_cast<float>(frame.extent.width), static_cast<float>(frame.extent.height));
