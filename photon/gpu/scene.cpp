...
TextureResource createTexture2DFromRGBA(Scene& scene, GPU& gpu, const unsigned char* rgba,
...
void createFallbackTexture(Scene& scene, GPU& gpu) {
  ...
  scene.fallbackWhiteTexture =
      createTexture2DFromRGBA(scene, gpu, white, 1, 1, VK_FORMAT_R8G8B8A8_SRGB);
}

const TextureResource* pickTexture(const Scene& scene, const SceneObject& object, int textureIndex,
                                   bool srgb) {
  const std::vector<TextureResource>& textures =
      srgb ? object.gltfTexturesSrgb : object.gltfTexturesLinear;
...
void loadObjectTextures(Scene& scene, SceneObject& object, GPU& gpu) {
  ...
  for (size_t i = 0; i < object.model.textures.size(); i++) {
    ...
    if (texture.source < 0 || texture.source >= static_cast<int>(object.model.images.size()))
      continue;
    ...
    object.gltfTexturesSrgb[i] =
        createTexture2DFromRGBA(scene, gpu, rgba.data(), static_cast<uint32_t>(image.width),
                                static_cast<uint32_t>(image.height), VK_FORMAT_R8G8B8A8_SRGB);
    object.gltfTexturesLinear[i] =
        createTexture2DFromRGBA(scene, gpu, rgba.data(), static_cast<uint32_t>(image.width),
...
void createObjectMaterialResources(Scene& scene, SceneObject& object, GPU& gpu) {
  ...
  std::vector<VkDescriptorSetLayout> layouts(object.materials.size(),
                                             scene.materialDescriptorSetLayout);
  ...
  for (size_t i = 0; i < object.materials.size(); i++) {
    ...
    void* mapped = nullptr;
    vkMapMemory(gpu.device, object.materialUniformMemories[i], 0, sizeof(MaterialParams), 0,
                &mapped);
    ...
    const TextureResource* baseColor =
        pickTexture(scene, object, object.materials[i].baseColorTextureIndex, true);
    const TextureResource* metallicRoughness =
        pickTexture(scene, object, object.materials[i].metallicRoughnessTextureIndex, false);
    const TextureResource* normal =
        pickTexture(scene, object, object.materials[i].normalTextureIndex, false);
    const TextureResource* occlusion =
        pickTexture(scene, object, object.materials[i].occlusionTextureIndex, false);
    const TextureResource* emissive =
        pickTexture(scene, object, object.materials[i].emissiveTextureIndex, true);
...
void destroyFrame(Scene& scene, uint32_t index) {
  ...
  if (frame.sceneFramebuffer != VK_NULL_HANDLE)
    vkDestroyFramebuffer(scene.device, frame.sceneFramebuffer, nullptr);
  if (frame.postFramebuffer != VK_NULL_HANDLE)
    vkDestroyFramebuffer(scene.device, frame.postFramebuffer, nullptr);
  if (frame.sceneMsaaColorView != VK_NULL_HANDLE)
    vkDestroyImageView(scene.device, frame.sceneMsaaColorView, nullptr);
  if (frame.sceneMsaaColorImage != VK_NULL_HANDLE)
    scene.gpu->destroyImage(frame.sceneMsaaColorImage);
  if (frame.sceneMsaaColorMemory != VK_NULL_HANDLE)
    scene.gpu->freeMemory(frame.sceneMsaaColorMemory);
  if (frame.sceneColorView != VK_NULL_HANDLE)
    vkDestroyImageView(scene.device, frame.sceneColorView, nullptr);
  ...
  if (frame.sceneDepthView != VK_NULL_HANDLE)
    vkDestroyImageView(scene.device, frame.sceneDepthView, nullptr);
  ...
  if (frame.outputView != VK_NULL_HANDLE)
    vkDestroyImageView(scene.device, frame.outputView, nullptr);
  ...
  if (frame.uniformMapped != nullptr && frame.uniformMemory != VK_NULL_HANDLE)
    vkUnmapMemory(scene.device, frame.uniformMemory);
...
void initFrame(Scene& scene, GPU& gpu, uint32_t index) {
  ...
  const VkImageAspectFlags depthAspect =
      hasStencilComponent(scene.sceneDepthFormat)
          ? (VK_IMAGE_ASPECT_DEPTH_BIT | VK_IMAGE_ASPECT_STENCIL_BIT)
          : VK_IMAGE_ASPECT_DEPTH_BIT;

  createImageResource(gpu, frame.extent, scene.sceneColorFormat,
                      VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT | VK_IMAGE_USAGE_TRANSIENT_ATTACHMENT_BIT,
                      VK_IMAGE_ASPECT_COLOR_BIT, scene.msaaSamples, frame.sceneMsaaColorImage,
  ...
  createImageResource(gpu, frame.extent, scene.sceneDepthFormat,
                      VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT, depthAspect, scene.msaaSamples,
  ...
  vkMapMemory(gpu.device, frame.uniformMemory, 0, sizeof(SceneViewProjection), 0,
              &frame.uniformMapped);
...
glm::vec3 readColorValue(const uint8_t* colorData, size_t colorStride, int colorType,
...
void loadObject(SceneObject& object) {
  ...
  if (!object.loader.LoadBinaryFromMemory(&object.model, &err, &warn, object.source.data(),
                                          static_cast<unsigned int>(object.source.size()),
                                          "assets/models")) {
    return;
  }
  ...
  for (const SceneObject& object : scene.objects) {
    ...
    for (const GltfVertex& vertex : object.vertices) {
      const glm::vec4 worldPos =
          modelMatrix * glm::vec4(vertex.pos[0], vertex.pos[1], vertex.pos[2], 1.0f);
...
void Scene::addModel(const char* name, const unsigned char* newModel, size_t size,
                     bool isTrackable) {
  SceneObject object{};
...
void Scene::prepareInit(GPU& gpu) {
  ...
  sceneStages[0] = gpu.loadShader(scene_vert_spv, scene_vert_spv_size, sceneVertModule,
                                  VK_SHADER_STAGE_VERTEX_BIT, device);
  sceneStages[1] = gpu.loadShader(scene_frag_spv, scene_frag_spv_size, sceneFragModule,
  ...
  postStages[0] = gpu.loadShader(postProcess_vert_spv, postProcess_vert_spv_size, postVertModule,
                                 VK_SHADER_STAGE_VERTEX_BIT, device);
  postStages[1] = gpu.loadShader(scenePostProcess_frag_spv, scenePostProcess_frag_spv_size,
                                 postFragModule, VK_SHADER_STAGE_FRAGMENT_BIT, device);
...
void Scene::render(GPU& gpu, VkCommandBuffer& commandBuffer) {
  ...
  vkCmdBindDescriptorSets(commandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, scenePipelineLayout, 0, 1,
  ...
  vkCmdPushConstants(commandBuffer, postPipelineLayout,
                     VK_SHADER_STAGE_VERTEX_BIT | VK_SHADER_STAGE_FRAGMENT_BIT, 0,
                     sizeof(ScenePushConstants), &postPushConstants);
...
void Scene::destroy() {
  ...
  for (SceneObject& object : objects) {
    for (size_t i = 0; i < object.materialUniformBuffers.size(); i++) {
      if (object.materialUniformBuffers[i] != VK_NULL_HANDLE)
        gpu->destroyBuffer(object.materialUniformBuffers[i]);
      if (object.materialUniformMemories[i] != VK_NULL_HANDLE)
        gpu->freeMemory(object.materialUniformMemories[i]);
    }
