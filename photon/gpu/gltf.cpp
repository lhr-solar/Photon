...
namespace tinygltf {
bool WriteImageData(const std::string*, const std::string*, Image*, bool, void*) {
    return false;
}
}
...
static void destroyTexture(TextureResource& texture, VkDevice device) {
...
static void destroyFrame(Gltf& gltf, uint32_t index) {
    ...
    if (frame.sceneDepthView != VK_NULL_HANDLE) vkDestroyImageView(gltf.device, frame.sceneDepthView, nullptr);
    if (frame.sceneDepthImage != VK_NULL_HANDLE) vkDestroyImage(gltf.device, frame.sceneDepthImage, nullptr);
    if (frame.sceneDepthMemory != VK_NULL_HANDLE) vkFreeMemory(gltf.device, frame.sceneDepthMemory, nullptr);
    if (frame.outputView != VK_NULL_HANDLE) vkDestroyImageView(gltf.device, frame.outputView, nullptr);
    if (frame.outputImage != VK_NULL_HANDLE) vkDestroyImage(gltf.device, frame.outputImage, nullptr);
    if (frame.outputMemory != VK_NULL_HANDLE) vkFreeMemory(gltf.device, frame.outputMemory, nullptr);
    if (frame.uniformMapped != nullptr && frame.uniformMemory != VK_NULL_HANDLE) vkUnmapMemory(gltf.device, frame.uniformMemory);
    if (frame.uniformBuffer != VK_NULL_HANDLE) vkDestroyBuffer(gltf.device, frame.uniformBuffer, nullptr);
    if (frame.uniformMemory != VK_NULL_HANDLE) vkFreeMemory(gltf.device, frame.uniformMemory, nullptr);
...
static TextureResource createTexture2DFromRGBA(Gltf& gltf, GPU& gpu,
    ...
    range.layerCount = 1;
    gpu.setImageLayout(commandBuffer, texture.image, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL,
    ...
    gpu.setImageLayout(commandBuffer, texture.image, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL,
...
static void createFallbackTexture(Gltf& gltf, GPU& gpu) {
    ...
    gltf.fallbackWhiteTexture = createTexture2DFromRGBA(gltf, gpu, white, 1, 1, VK_FORMAT_R8G8B8A8_SRGB);
}
...
static void loadGltfTextures(Gltf& gltf, GPU& gpu) {
    ...
    for (size_t i = 0; i < gltf.model.textures.size(); i++) {
        ...
        if (texture.source < 0 || texture.source >= static_cast<int>(gltf.model.images.size())) continue;
        ...
        gltf.gltfTexturesSrgb[i] = createTexture2DFromRGBA(gltf, gpu, rgba.data(),
            static_cast<uint32_t>(image.width), static_cast<uint32_t>(image.height), VK_FORMAT_R8G8B8A8_SRGB);
        gltf.gltfTexturesLinear[i] = createTexture2DFromRGBA(gltf, gpu, rgba.data(),
...
static const TextureResource* pickTexture(const Gltf& gltf, int textureIndex, bool srgb) {
    const std::vector<TextureResource>& textures = srgb ? gltf.gltfTexturesSrgb : gltf.gltfTexturesLinear;
...
static void createVertexBuffer(Gltf& gltf, GPU& gpu) {
    ...
    const VkDeviceSize bufferSize = static_cast<VkDeviceSize>(gltf.vertices.size() * sizeof(GltfVertex));
...
static void createMaterialResources(Gltf& gltf, GPU& gpu) {
    ...
    std::vector<VkDescriptorSetLayout> layouts(gltf.materials.size(), gltf.materialDescriptorSetLayout);
    VkDescriptorSetAllocateInfo allocInfo{};
    allocInfo.sType = VK_STRUCTURE_TYPE_DESCRIPTOR_SET_ALLOCATE_INFO;
    allocInfo.descriptorPool = gltf.internalDescriptorPool;
    allocInfo.descriptorSetCount = static_cast<uint32_t>(layouts.size());
    allocInfo.pSetLayouts = layouts.data();
    vkAllocateDescriptorSets(gpu.device, &allocInfo, gltf.materialDescriptorSets.data());
    ...
    for (size_t i = 0; i < gltf.materials.size(); i++) {
        ...
        const TextureResource* baseColor = pickTexture(gltf, gltf.materials[i].baseColorTextureIndex, true);
        const TextureResource* metallicRoughness = pickTexture(gltf, gltf.materials[i].metallicRoughnessTextureIndex, false);
        const TextureResource* normal = pickTexture(gltf, gltf.materials[i].normalTextureIndex, false);
        const TextureResource* occlusion = pickTexture(gltf, gltf.materials[i].occlusionTextureIndex, false);
        const TextureResource* emissive = pickTexture(gltf, gltf.materials[i].emissiveTextureIndex, true);
...
static void initFrame(Gltf& gltf, GPU& gpu, uint32_t index) {
    ...
    const VkImageAspectFlags depthAspect = hasStencilComponent(gltf.sceneDepthFormat)
...
static glm::vec3 readColorValue(const uint8_t* colorData, size_t colorStride,
...
static void emitPrimitiveVertex(const uint8_t* posData, size_t posStride, const uint8_t* normalData,
    ...
    glm::vec3 color = readColorValue(colorData, colorStride, colorType, colorComponentType, colorNormalized, hasColor, vertexIndex);
...
void Gltf::init(GPU& gpu, const unsigned char* newModel, size_t size){
    ...
    for (const PrimitiveRange& range : ranges) {
        ...
        item.materialIndex = range.materialIndex >= 0 && range.materialIndex < static_cast<int>(materials.size())
            ? static_cast<uint32_t>(range.materialIndex) : 0u;
    ...
    gltfStages[0] = gpu.loadShader(gltf_vert_spv, gltf_vert_spv_size, gltfVertModule, VK_SHADER_STAGE_VERTEX_BIT, device);
    gltfStages[1] = gpu.loadShader(gltf_frag_spv, gltf_frag_spv_size, gltfFragModule, VK_SHADER_STAGE_FRAGMENT_BIT, device);
...
void Gltf::render(GPU& gpu, VkCommandBuffer& commandBuffer){
    ...
    GltfPushConstants pushConstants{{static_cast<float>(frame.extent.width), static_cast<float>(frame.extent.height)}, time, 0.0f};
    vkCmdPushConstants(commandBuffer, gltfPipelineLayout, VK_SHADER_STAGE_VERTEX_BIT | VK_SHADER_STAGE_FRAGMENT_BIT,
        0, sizeof(GltfPushConstants), &pushConstants);
    ...
    for (const DrawItem& item : drawItems) {
        ...
        vkCmdBindDescriptorSets(commandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, gltfPipelineLayout,
            1, 1, &materialDescriptorSets[item.materialIndex], 0, nullptr);
    ...
    vkCmdSetViewport(commandBuffer, 0, 1, &viewport);
    vkCmdSetScissor(commandBuffer, 0, 1, &scissor);
    vkCmdPushConstants(commandBuffer, postPipelineLayout, VK_SHADER_STAGE_VERTEX_BIT | VK_SHADER_STAGE_FRAGMENT_BIT,
        0, sizeof(GltfPushConstants), &pushConstants);
...
void Gltf::destroy(){
    ...
    for (size_t i = 0; i < materialUniformBuffers.size(); i++) {
        if (materialUniformBuffers[i] != VK_NULL_HANDLE) vkDestroyBuffer(device, materialUniformBuffers[i], nullptr);
        if (materialUniformMemories[i] != VK_NULL_HANDLE) vkFreeMemory(device, materialUniformMemories[i], nullptr);
    }
    ...
    for (TextureResource& texture : gltfTexturesSrgb) destroyTexture(texture, device);
    for (TextureResource& texture : gltfTexturesLinear) destroyTexture(texture, device);
    ...
    if (gltfPipelineLayout != VK_NULL_HANDLE) vkDestroyPipelineLayout(device, gltfPipelineLayout, nullptr);
