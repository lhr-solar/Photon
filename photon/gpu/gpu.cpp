...
void Gpu::setupLayoutsAndDescriptors(VkDevice device)
{
    ...
    std::vector<VkDescriptorSetLayoutBinding> setLayoutBindings = {uniformLayoutBinding, materialLayoutBinding, meshLayoutBinding};
...
{
    for (size_t modelIdx = 0; modelIdx < gltfLoader.getModelCount(); ++modelIdx)
    {
        Model *model = gltfLoader.getModel(modelIdx);
        if (!model)
            continue;
        ...
        for (Model::Mesh mesh : model->meshes)
        {
            ...
            // set up descriptor for materials
            if (mesh.shaderMaterialBuffer.buffer != VK_NULL_HANDLE)
            {
            ...
            // animations
            if (mesh.shaderMeshBuffer.buffer != VK_NULL_HANDLE)
            {
                ...
                writes.push_back(meshWrite);
            }
            // Update descriptors for mesh
            vkUpdateDescriptorSets(vulkanDevice.logicalDevice,
                                   static_cast<uint32_t>(writes.size()),
                                   writes.data(), 0, nullptr);
        }
        logs("[+] setup descriptor for " + model->name);
    }
}
